...
namespace Zobrist {
    ...
    uint64_t castlingKeys[4]; // Standard Polyglot uses 4 keys, not 16
    uint64_t epKeys[8];        // 8 files (a-h)
    uint64_t sideKey;          // Side to move

    // Place your standard Polyglot constant array here (abbreviated for length)
    ...
    {
        switch (piece) {
            case Piece::BlackPawn:   return 0;
            case Piece::WhitePawn:   return 1;

            case Piece::BlackKnight: return 2;
            case Piece::WhiteKnight: return 3;

            case Piece::BlackBishop: return 4;
            case Piece::WhiteBishop: return 5;

            case Piece::BlackRook:   return 6;
            case Piece::WhiteRook:   return 7;

            case Piece::BlackQueen:  return 8;
            case Piece::WhiteQueen: return 9;
    ...
    void init() {
            ...
            // 1. Initialize piece keys (12 pieces * 64 squares = 768 elements)
            for (int piece = 0; piece < 12; ++piece) {
                for (int sq = 0; sq < 64; ++sq) {
                    pieceKeys[piece][sq] = Random64[index++];
                }
            }

            // 2. Initialize castling rights keys (4 standard Polyglot keys)
            for (int i = 0; i < 4; ++i) {
                castlingKeys[i] = Random64[index++];
            }

            // 3. Initialize en passant file keys (8 files)
            for (int file = 0; file < 8; ++file) {
                epKeys[file] = Random64[index++];
            }

            // 4. Initialize side to move key (1 element)
            sideKey = Random64[index++];
        }
}
